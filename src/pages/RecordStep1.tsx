...
import { matchStockSearch } from '../utils/search';
...
export default function RecordStep1() {
  ...
  // Filter suggestions (최대 50개 노출)
  const suggestions = stockName.trim().length > 0
    ? POPULAR_STOCKS.filter(s => matchStockSearch(stockName.trim(), s)).slice(0, 50)
    : [];
  ...
  const handleSelectSuggestion = (stock: string) => {
    setStockName(stock);
    setShowDropdown(false);
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pt-12 p-6">
      ...
      <section className="flex-1 space-y-8">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mb-3">리스트에 없는 종목도 자유롭게 입력할 수 있습니다. (초성 검색 지원)</p>
          <div className="relative" ref={dropdownRef}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3 top-3.5 text-slate-400 z-10 pointer-events-none"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            <input 
              type="text" 
              value={stockName}
              onChange={(e) => {
              ...
              placeholder="종목명을 입력하세요 (예: 삼성전자, ㅅㅅㅈㅈ, TSLA)" 
            ...
            {showDropdown && suggestions.length > 0 && (
              <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden max-h-52 overflow-y-auto">
                {suggestions.map(stock => (
                  <li
                    ...
                    className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-blue-50 hover:text-blue-700 cursor-pointer transition-colors border-b border-slate-50 last:border-b-0"
                  >
                    <span className="shrink-0 text-lg leading-none">{stock.slice(0, 2)}</span>
                    <span className="truncate">{stock.slice(3)}</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['뉴스/공시', '유튜브/지인 추천', '가격 급락', '차트/수급', '기타 (직접 입력)'].map(tag => (
              <button 
                key={tag} 
